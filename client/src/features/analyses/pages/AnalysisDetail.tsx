...
export default function AnalysisDetail() {
  ...
  useEffect(() => {
    ...
    const poll = window.setInterval(() => {
      void getAnalysisById(analysisId)
        ...
        .catch((err) => {
          setRefreshWarning(
            getApiErrorMessage(err, "Could not refresh analysis status."),
          );
        });
    }, 3000);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        if (!cancelled) {
          setRefreshWarning(
            getApiErrorMessage(err, "Could not refresh analysis."),
          );
        }
      }
    };
  ...
  return (
    <div className="app-page pt-8">
      ...
      <div className="glass-card glass-card-elevated mx-auto flex max-w-225 items-center justify-between px-8 py-4">
        <button
          type="button"
      ...
      <div className="glass-card glass-card-elevated mx-auto my-8 max-w-225 px-8 py-6">
        {loading && (
          <p className="animate-pulse p-8 text-center text-slate-500">
        ...
        {!loading && data && refreshWarning && (
          <p className="alert alert-warning mb-4">{refreshWarning}</p>
        )}

        {!loading && data && (
          <>
            {/* Info Grid */}
            <div className="mb-8 grid grid-cols-2 gap-4 border-b border-slate-200/60 pb-6">
              <div>
                <div className="form-label-sm mb-2">General Info</div>
                <div className="mb-1 text-[0.9rem] text-slate-700">
                  <span className="mr-2 font-semibold text-slate-500">
                    Request:
                  </span>
                  {data.request_title?.trim() || "Untitled"}
                </div>
                <div className="mb-1 text-[0.9rem] text-slate-700">
                  <span className="mr-2 font-semibold text-slate-500">
                    Status:
              ...
              <div>
                <div className="form-label-sm mb-2">History</div>
                <div className="mb-1 text-[0.9rem] text-slate-700">
                  <span className="mr-2 font-semibold text-slate-500">
                    Created:
                  </span>
                ...
                <div className="mb-1 text-[0.9rem] text-slate-700">
                  ...
                  {data.issued_by?.username || "System"}
                </div>
              </div>
            </div>

            <div className="mb-6 flex items-center rounded-lg border border-slate-200/60 bg-slate-50/80 p-4 shadow-sm">
              <span className="mr-3 font-bold text-slate-700">
                Analysis Status:
              </span>

              {(() => {
                const badge = getAnomalyBadge(
                  data.status,
              ...
              {data.before_image && (
                <div className="group">
                  ...
                  <img
                    ...
                    alt="State Before"
                    className="max-h-112.5 w-full rounded-xl border border-slate-200 object-cover shadow-md transition-all duration-300 hover:scale-[1.005] hover:shadow-lg"
                  />
                </div>
              )}
