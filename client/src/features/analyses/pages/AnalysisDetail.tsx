...
export default function AnalysisHistory() {
    ...
    // קביעת תאריך המקסימום להיום (2026-04-20 לפי המערכת)
    const today = new Date().toISOString().split('T')[0];

    // פונקציית טעינת נתונים מרוכזת
    const fetchData = useCallback(async () => {
        // בדיקת לוגיקה תאריכים: אם תאריך ההתחלה מאוחר מתאריך הסיום
    ...
    const handleBulkExport = async (format: 'PDF' | 'EXCEL') => {
        if (startDate && endDate && startDate > endDate) {
            alert("Check your dates: Start date is after End date.");
            return;
        }
        setExporting(format);
        try {
        ...
        <div className="min-h-screen bg-[#f0f4f8] pt-8 [font-:'Segoe_UI',system-ui,sans-serif]">
            ...
            <div className="mx-auto mb-6 max-w-[1100px] border-b border-t border-[#e2e8f0] bg-white px-8 py-4 shadow-sm flex justify-between items-center rounded-lg">
                ...
                <div className="text-right">
                    ...
                    <p className="text-xs text-[#64748b]">Filter and search historical data</p>
                </div>
            </div>
                ...
                <div className="flex flex-wrap items-end gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <select
                            ...
                            className="px-3 py-2 border border-[#e2e8f0] rounded-lg text-sm bg-white outline-none focus:ring-2 focus:ring-[#3b82f6]"
                        >
                    ...
                    <button
                        onClick={fetchData}
                        className="px-6 py-2 bg-[#1e293b] text-white rounded-lg text-sm font-bold hover:bg-[#334155] transition-all shadow-md active:scale-95"
                    >
                        🔍 Search
                    </button>
                    ...
                    <div className="flex gap-2 ml-auto">
                        <button
                            ...
                            className="min-w-[120px] px-3 py-2 bg-red-50 text-red-600 border border-red-100 rounded-lg text-xs font-bold hover:bg-red-100 disabled:opacity-50 transition-all"
                        >
                        ...
                        <button
                            ...
                            className="min-w-[120px] px-3 py-2 bg-green-50 text-green-600 border border-green-100 rounded-lg text-xs font-bold hover:bg-green-100 disabled:opacity-50 transition-all"
                        >
                    ...
                    <div className="flex flex-col gap-3">
                        {analyses.map((item) => (
                            ...
                            >
                                ...
                                <div className="text-[#94a3b8] group-hover:text-[#3b82f6] transition-colors">Details →</div>
                            </div>
                        ))}
                    </div>
                )}
