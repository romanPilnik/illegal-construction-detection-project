    ...
    useEffect(() => {
        ...
        ;(async () => {
            try {
                ...
                const payload = res.data as AnalysisDetailResponse
                ...
                {!loading && !error && data && (
                    <>
                        <div className="mb-3 text-[0.9rem] text-[#334155]">
                            <span className="mr-2 font-semibold text-[#64748b]">ID</span>{data.id}
                        </div>
                        <div className="mb-3 text-[0.9rem] text-[#334155]">
                            <span className="mr-2 font-semibold text-[#64748b]">Status</span>{data.status}
                        </div>
                        <div className="mb-3 text-[0.9rem] text-[#334155]">
                            <span className="mr-2 font-semibold text-[#64748b]">Created</span>{new Date(data.created_at).toLocaleString()}
                        </div>
                        <div className="mb-3 text-[0.9rem] text-[#334155]">
                            <span className="mr-2 font-semibold text-[#64748b]">Anomaly detected</span>{data.anomaly_detected === null ? '—' : String(data.anomaly_detected)}
                        </div>
                        {data.issued_by && (
                            <div className="mb-3 text-[0.9rem] text-[#334155]">
                                <span className="mr-2 font-semibold text-[#64748b]">Issued by</span>{data.issued_by.username}
                            </div>
                        )}
                        {data.before_image && (
                            <div className="mb-3 text-[0.9rem] text-[#334155]">
                                <span className="mr-2 font-semibold text-[#64748b]">Before</span>{data.before_image.file_path}
                            </div>
                        )}
                        {data.after_image && (
                            <div className="mb-3 text-[0.9rem] text-[#334155]">
                                <span className="mr-2 font-semibold text-[#64748b]">After</span>{data.after_image.file_path}
                            </div>
                        )}
                        {data.result_image && (
                            <div className="mb-3 text-[0.9rem] text-[#334155]">
                                <span className="mr-2 font-semibold text-[#64748b]">Result</span>{data.result_image.file_path}
                            </div>
                        )}
                    </>
                )}
